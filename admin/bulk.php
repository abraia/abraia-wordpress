      ...
      <script type="text/javascript">
        jQuery(document).ready(function($) {
          ...
          function updateProgress(sum, total) {
            ...
            $('.progress-right .progress-bar').css({'transform': 'rotate(' + ((sum > total / 2) ? 180 : Math.round(360 * sum / total)) + 'deg)'});
            $('.progress-left .progress-bar').css({'transform': 'rotate(' + ((sum > total / 2) ? Math.round(360 * sum / total - 180) : 0) + 'deg)'});
          ...
          function startWorker() {
            return $.when().then(function next() { return nextTask().then(next); })
          }
          var bulkButton = $('#bulk');
